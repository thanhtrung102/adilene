...
const s_filteredWords = ['tranny', 'troon', 'trannie', 'fag', 'faggot', 'nigga', 'nigger', 'negro', 'coon', 'coons', 'retarded', 'retard', 'beaner', 'beaners', 'spic', 'wetback', 'incest', 'pedo', 'pedophile', 'loli', 'lolicon', 'shota', 'shotacon', 'lolisho', 'pussy', 'vagina', 'dick', 'penis', 'cock', 'porn', 'pornography', 'porno', 'rape', 'rapist', 'sex', 'sexual', 'sexually', 'sexuality', 'cum', 'semen', 'slut', 'slutty', 'tits', 'titties', 'titty', 'whore', 'DEI', 'bitch', 'trap', 'shemale', 'boob', 'boobs', 'boobies'];
...
const v_mainHtml = `
    <div id="c_container">
        <div id="c-wrapper" class="c-wrapper"><div style="text-align:center;margin-top:10px;">${s_loadingText}</div></div>
    </div>
...
const v_formHtml = `
    ...
    <div id="c_textWrapper" class="c-inputWrapper">
        ...
        <i class="fas fa-smile" id="toggleEmojiButton" onclick="toggleEmojiPicker();"></i>
    </div>
...
const c_wrapper = document.getElementById("c-wrapper");
let v_pageNum = 1;
...
c_hiddenIframe.id = 'c_hiddenIframe'; c_hiddenIframe.name = 'c_hiddenIframe'; c_hiddenIframe.style.display = 'none'; c_hiddenIframe.setAttribute('onload', 'if(v_submitted){fixFrame();$("#c-wrapper").animate({ scrollTop: 0 }, "fast");}');
...
function getComments(){
    ...
    retrievedSheet.then(result =>{
        ...
        if (comments.length == 0 || Object.keys(comments[0]).length < 2){ // Once again, Google Sheets can be weird
            c_wrapper.innerHTML = s_noCommentsText;
...
function displayComments(comments){
    ...
    if (v_amountOfPages > 1){
        ...
        c_wrapper.appendChild(pagination);
    }
    
    // Emoji window
    let emojiWindow = document.createElement('div');
...
function toggleEmojiPicker() {
    const picker = document.getElementById('c-emojiPicker');
...
function addEmoji(emoji){
    const emojiMap = {
        ...
        'emoji2': 'miku_cheer',
        'emoji3': 'miku_nice',
        'emoji4': 'miku_celebration',
        'emoji5': 'miku_love',
        'emoji6': 'miku_exhausted',
        'emoji7': 'rin_amazed',
        'emoji8': 'rin_angry',
        'emoji9': 'rin_yes',
        'emoji10': 'rin_snooze',
        ...
        'emoji15': 'luka_byebye',
        'emoji16': 'luka_blegh',
        'emoji17': 'luka_confused',
        'emoji18': 'meiko_ok',
        'emoji19': 'meiko_drink',
        'emoji20': 'kaito_cry',
        'emoji21': 'kaito_despair',
        'emoji22': 'kaito_nogood'
    };
...
function createComment(data){
    ...
    filteredText = filteredText.replace(/:(miku_39|miku_cheer|miku_nice|miku_celebration|miku_love|miku_exhausted|rin_amazed|rin_angry|rin_yes|rin_snooze|len_shy|len_shocked|len_no|len_plead|luka_byebye|luka_blegh|luka_confused|meiko_ok|meiko_drink|kaito_cry|kaito_despair|kaito_nogood):/g, function(match, p1) {
        const emojiMap = {
            'miku_39': 'miku_39',
            'miku_cheer': 'miku_cheer',
            'miku_nice': 'miku_nice',
            'miku_celebration': 'miku_celebration',
            'miku_love': 'miku_love',
            'miku_exhausted': 'miku_exhausted',
            'rin_amazed': 'rin_amazed',
            'rin_angry': 'rin_angry',
            'rin_yes': 'rin_yes',
            'rin_snooze': 'rin_snooze',
            'len_shy': 'len_shy',
            'len_shocked': 'len_shocked',
            'len_no': 'len_no',
            'len_plead': 'len_plead',
            'luka_byebye': 'luka_byebye',
            'luka_blegh': 'luka_blegh',
            'luka_confused': 'luka_confused',
            'meiko_ok': 'meiko_ok',
            'meiko_drink': 'meiko_drink',
            'kaito_cry': 'kaito_cry',
            'kaito_despair': 'kaito_despair',
            'kaito_nogood': 'kaito_nogood'
        };

        const emojiName = emojiMap[p1];
        if (emojiName) {
            return `<img src="/images/emojis/${emojiName}.gif" class="c-emoji" alt="${emojiName}">`;
        } else {
            return match;
        }
    });
...
function changePage(dir){
    ...
    $("#c-wrapper").animate({ scrollTop: 0 }, "fast");
}
